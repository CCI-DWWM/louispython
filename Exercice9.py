...
conn = sqlite3.connect("departements.db")
cursor = conn.cursor()
...
cursor.executemany("INSERT OR IGNORE INTO departements (numero, nom) VALUES (?, ?)", departements)
conn.commit()
conn.close()

# Fonction pour récupérer le nom d'un département via son numéro
def get_nom_departement(numero):
    conn = sqlite3.connect("departements.db")
    cursor = conn.cursor()
    cursor.execute("SELECT nom FROM departements WHERE numero = ?", (numero,))
    result = cursor.fetchone()
    conn.close()
    return result[0] if result else "Département inconnu"
